...
describe('lola', () => {
  ...
  describe('constructor', () => {
    test('Should instantiate konekto and server on constructor', () => {
      const lola = new Lola({ jwtConfig, rbacOptions: {}, connectionConfig: 'xd' })
  ...
  describe('seed', () => {
    let lola
    beforeEach(async () => {
      lola = new Lola({ jwtConfig, rbacOptions: {}, connectionConfig: 'xd' })
    })
    test('Should throw error if graphName is not a string', async () => {
      await expect(lola.seed({ graphName: 1, schema: { _label: 'nice' } })).rejects.toThrow(
        'graphName must be a string'
      )
    })
    test('Should throw error if graphName is an empty string', async () => {
      await expect(lola.seed({ graphName: '', schema: { _label: 'nice' } })).rejects.toThrow(
        'graphName must be a string'
      )
    })
    test('Should successfully call seed with the correct parameters', async () => {
      await expect(lola.seed({ graphName: 'xd', schema: { _label: 'nice' } })).resolves.toBe(undefined)
    })
    test('Should call seedFn passing konekto as first parameter', async () => {
      const seedFn = jest.fn()
      await lola.seed({ graphName: 'xd', schema: { _label: 'nice' }, seedFn })
    ...
    test('Should set _seeded to true after finishing seed without seed function', async () => {
      await lola.seed({ graphName: 'xd', schema: { _label: 'nice' } })
      expect(lola._seeded).toBe(true)
    })
  })
  describe('start', () => {
    let lola
    beforeEach(async () => {
      lola = new Lola({ jwtConfig, rbacOptions: {} })
    })
    ...
    test('Should call lola.server.listen with the parameters passed to lola.listen', async () => {
      await lola.seed({ graphName: 'lola_test', schema: { _label: 'hehe' } })
